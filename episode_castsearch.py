...
def find_similar_titles(user_input, df):
    ...
    similar_titles = [
        title for title in title_choices if fuzz.partial_ratio(user_input, title) >= 80
    ]
...
def index():
    ...
    if request.method == "POST":
        ...
        if not filtered_df.empty:
            ...
            combined_output = (
                search_form + details_output + summary_output + cast_output
            )
        else:
            ...
            if similar_titles:
                ...
                combined_output = (
                    search_form
                    + f"No exact match found for '{filter_value}'. {similar_titles_list}"
                )
            else:
                combined_output = (
                    search_form
                    + f"No rows found with a title similar to '{filter_value}'"
                )
